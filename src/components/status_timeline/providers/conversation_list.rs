//! Conversation list provider - displays agent conversations as timeline
...
impl ConversationListProvider {
    pub fn new(environment: Environment) -> Self {
        Self { environment }
    }
}

impl std::fmt::Debug for ConversationListProvider {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ConversationListProvider").finish()
    }
}

impl TimelineProvider for ConversationListProvider {
    type Id = StatusId; // Conversation ID mapped to StatusId
    ...
    fn should_auto_reload(&self) -> bool {
        false // User manually refreshes conversation list
    }
    ...
    fn forced_direction(&self) -> Option<TimelineDirection> {
        Some(TimelineDirection::NewestTop) // Newest conversations at top
    }
    ...
    fn scroll_to_item(&self, updates: &[crate::environment::model::Status]) -> Option<StatusId> {
        ...
        updates
            ...
            .map(|status| {
                log::debug!("Conversation list scrolling to status: {}", status.id);
    ...
    {
        // Note: Database::list_conversations() doesn't support pagination yet
        // Always returns full list ordered by last_message_at DESC
        ...
        Box::pin(async move {
            ...
            Ok(summaries
                .iter()
                .map(conversation_summary_to_status)
                .collect())
        })
    }
...
/// Transform ConversationSummary to Status for timeline system
fn conversation_summary_to_status(
