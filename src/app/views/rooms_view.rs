...
#[component]
pub fn RoomsView(auth_state: AuthState) -> Element {
    use crate::components::status_timeline::{
        Action, State as TimelineState, TimelineComponent, handle_action,
    };
    use crate::components::status_timeline::{AnyTimelineProvider, RoomListProvider};
    ...
    // Create room list provider
    let provider = AnyTimelineProvider::new(
        RoomListProvider::new(environment.clone()),
        &account.id
    );
