...
impl ToolbarDelegate for LoggedOutBar {
    const NAME: &'static str = "CYRUPToolbarLoggedout";
    ...
    fn item_for(&self, identifier: &str) -> &ToolbarItem {
        // LoggedOutBar should never be asked for toolbar items
        panic!(
            "LoggedOutBar::item_for called with identifier '{}' - this indicates an architectural error",
            identifier
        );
    }
}
...
impl LoggedInToolbar {
    pub fn new(
        ...
        let url = if url::Url::parse(&account_url).is_ok() {
            ...
            NSURL::with_str("https://terhech.de/cyrup/images/logo2.png")
        };
        ...
        account_item.set_action(move |_| {
            (*cloned)(AppEvent::MenuEvent(MainMenuEvent::More));
        });
...
impl ToolbarDelegate for LoggedInToolbar {
    const NAME: &'static str = "CYRUPToolbarLoggedIn";
    ...
    fn item_for(&self, identifier: &str) -> &ToolbarItem {
        match identifier {
            ...
            _ => {
                // Unknown toolbar identifier is a programming error
                panic!(
                    "LoggedInToolbar::item_for called with unknown identifier: '{}'. Valid identifiers are: {}, {}, {}, {}",
                    identifier, ACCOUNT_BUTTON, SEGMENTED_CONTROL, NEW_TOOT_BUTTON, RELOAD_BUTTON
                );
            }
        }
    }
